...
app.post('/api/generate-architecture', async (req, res) => {
  try {
    ...
    if (!formData.idea || !formData.userCount || !formData.compliance || !formData.skillLevel || !formData.timeline) {
      return res.status(400).json({ error: 'All fields are required' });
    }

    console.log('Generating architecture for:', formData.idea.substring(0, 50) + '...');
    ...
    const message = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: createArchitecturePrompt(formData)
      }]
    });
    ...
    try {
      ...
      architectureData = JSON.parse(responseText);
    } catch (parseError) {
      // If parsing fails, try to extract JSON from markdown code blocks
      const jsonMatch = responseText.match(/```json\s*([\s\S]*?)\s*```/) || responseText.match(/```\s*([\s\S]*?)\s*```/);
      if (jsonMatch) {
        architectureData = JSON.parse(jsonMatch[1]);
      } else {
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', message: 'Archie is ready to architect!' });
});
